...
public class SlotMachineMgr : MonoBehaviour
{
    ...
    {
        public List<Image> SlotSpirte = new List<Image>();
    }
    ...
    void Start()
    {
        ...
        for(int i = 0; i<Slot.Length; i++)
        {
            for(int j = 0; j<ItemCnt; j++)
            {
                ...
                DisplayItemSlots[i].SlotSpirte[j].sprite = skillSprite[StartList[randomIndex]];
                if(j==0)
                {
                    DisplayItemSlots[i].SlotSpirte[ItemCnt].sprite = skillSprite[StartList[randomIndex]];
                }
    ...
    {
        for (int i = 0; i < (ItemCnt * 10 + 1) * 2; i++)
        {
